...
export const addDetails = async (req, res) => {
  try {
    ...
    return res
      .status(201)
      .json({ message: "Appointment created successfully", careCustomer });
  } catch (err) {
    console.error(err);
...
export const updateUser = async (req, res) => {
  try {
    ...
    const careCustomer = await CareCustomer.findByIdAndUpdate(id, updateData, {
      new: true,
      runValidators: true,
    });
...
export const updateStatus = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .json({ message: `Status updated to ${status}`, careCustomer });
  } catch (err) {
    console.error(err);
    return res
      .status(500)
...
export const getByStatus = async (req, res) => {
  try {
    ...
    const careCustomers = await CareCustomer.find({ status }).sort({
      createdAt: -1,
    });
    ...
    if (!careCustomers || careCustomers.length === 0) {
      return res
        .status(404)
        .json({ message: `No ${status} appointments found` });
    }
