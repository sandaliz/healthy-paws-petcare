...
function resolveFocusPosition(doc, position = null) {
  ...
  const selectionAtStart = import_state3.Selection.atStart(doc);
  const selectionAtEnd = import_state3.Selection.atEnd(doc);
  ...
  if (position === "all") {
    return import_state3.TextSelection.create(doc, minMax(0, minPos, maxPos), minMax(doc.content.size, minPos, maxPos));
  }
  return import_state3.TextSelection.create(doc, minMax(position, minPos, maxPos), minMax(position, minPos, maxPos));
}
...
function selectionToInsertionEnd(tr, startLen, bias) {
  ...
  tr.setSelection(import_state4.Selection.near(tr.doc.resolve(end), bias));
}
...
function inputRulesPlugin(props) {
  ...
  const plugin = new import_state5.Plugin({
    state: {
      init() {
        return null;
      },
...
function pasteRulesPlugin(props) {
  ...
  const plugins = rules.map((rule) => {
    return new import_state6.Plugin({
  ...
  addProseMirrorPlugins() {
    return [
      new import_state7.Plugin({
        key: new import_state7.PluginKey("clipboardTextSerializer"),
        props: {
...
var extendMarkRange = (typeOrName, attributes = {}) => ({ tr, state, dispatch }) => {
  ...
  if (dispatch) {
    ...
    if (range && range.from <= from && range.to >= to) {
      const newSelection = import_state9.TextSelection.create(doc, range.from, range.to);
...
var selectAll = () => ({ tr, dispatch }) => {
  if (dispatch) {
    const selection = new import_state10.AllSelection(tr.doc);
    tr.setSelection(selection);
  }
  return true;
};
