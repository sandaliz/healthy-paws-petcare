...
import "./AppHisDetailsDC.css";

function AppHisDetailsDC() {
  const { id } = useParams();
  ...
  const fetchAppointment = useCallback(async () => {
    try {
      ...
      console.error("Error fetching appointment history details:", err);
  ...
  return (
    <Modal isOpen={true} onClose={() => navigate(-1)}>
      {appointment && (
        <div className="apphis-dc-container">
          <h2>Appointment History Details</h2>
...
export default AppHisDetailsDC;
